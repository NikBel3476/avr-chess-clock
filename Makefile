...
all: clean $(APP_NAME).o
	avr-objcopy -j .text -j .data -O ihex $(APP_NAME).o $(APP_NAME).hex
# $(AVROBJCOPY) -j .text -j .data -O ihex $(APP_NAME).o $(APP_NAME).hex
	
$(APP_NAME).o: $(source_files)
	avr-gcc $(FLAGS) $^ -S -o $(APP_NAME).asm
	avr-gcc $(FLAGS) $^ -o $(APP_NAME).o
# $(AVRGCC) $(FLAGS) $^ -S -o $(APP_NAME).asm
# $(AVRGCC) $(FLAGS) $^ -o $(APP_NAME).o
#Загрузка программы на МК
flush:
	avrdude -c $(PROGRAMMER) -P COM4 -p $(MMCU_PROG) -U flash:w:$(APP_NAME).hex
